  ...
  return new Promise(async (resolve, reject) => {
    if (config.sourceType === 'file') {
      ...
      if (config.userPassword) {
        try {
          const userWords = await provideUserWords(
            config.userWords,
            config.userPassword
          );
          const transformedWords: string[] =
...
export function createNitro2FAContext() {
  ...
  function displayTransformationInfo() {
    ...
    switch (transformation) {
      ...
      case 'count-consonants':
        return `IF I COUNT THE CONSONANTS IN THE WORD IN THIS POSITION ${
          randomIndex + 1
        }, THE VALUE IS: ${word}`;
      case 'reverse':
  ...
  async function nitro2FA(
    ...
    const answerToString =
      typeof userAnswer === 'number' ? String(userAnswer) : userAnswer;
